...
%               le nombre de Courant C, on choisit un C fixe de 0.8. On 
...
%
% Exemple: 8 noeuds initial, 6 raffinements, temps final de 2 secondes
%          >> WaveVerifConvergence(8, 6, 2)
%
% Historique
...
function WaveVerifConvergence(Ninit, Nraf, tend)
...
C = 0.8;           % Nombre de courant fixé à 0.8
...
% Terme source
Sxt = @(x,t) (b*sin(t)*(k*L*x^2-k*x^3)+cos(t)*(k*L*x^2-k*x^3 +T/rho*(2*k*L-6*k*x)));

% Solution analytique
%u_ana = @(x,t) sin(pi*x/L)*cos(pi*c*t/L);
u_ana = @(x,t) cos(t)*(k*x^3-k*L*x^2);
...
title(sprintf("Convergence Observée\n " + ...
   "Temps Final=%G s, Nombre de Courant C=%.1f, Fréquence=%.1f Hz", ...
   tend, C, c/(2*L)));
%xlabel('h=\Delta{x}=\Delta{t}C^{-1}(T/\rho)^{1/2} [m]');
...
triang_y = triang_y - 2*[h(b)^p_hat_L1(a) h(a)^p_hat_L1(a)];
